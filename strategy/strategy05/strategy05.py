...
class LSTMTrainer:
    ...
    def train(self, df_features, df_target, seq_len=30, verbose=True):
        df = df_features.copy()
        df['target'] = df_target
        df = df.dropna()
        ...
        X_all = torch.tensor(features, device=device)  # 常驻显存
        ...
        with torch.no_grad(), autocast(device.type):
            all_preds = []
            for start in range(0, total_samples, self.batch_size * 4):
                end = start + self.batch_size * 4
                batch_idx = torch.arange(start, min(end, total_samples), device=device)
                xb = torch.stack([X_all[i:i+seq_len] for i in batch_idx])
                pred = self.model(xb)
...
class MLStrategy:
    ...
    def exec_fn(self, ctx: pybroker.context.ExecContext):
        ...
        seq = self.feature_buffer.unsqueeze(0)  # 已经在 GPU 上
...
def main():
    ...
    print("atr_stop:", atr_stop)
    print("atr_profit:", atr_profit)
