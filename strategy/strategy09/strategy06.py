    ...
    def train(self, df_features, df_target, seq_len=40, verbose=True):
        ...
        with torch.no_grad():
            for start in range(0, total_samples, self.batch_size):
                ...
                xb = torch.stack([X_all[i:i+seq_len] for i in idx])
                pred = self.model(xb)
                all_preds.append(pred.cpu().numpy())
        all_preds = np.concatenate(all_preds)
        self.buy_thresh = np.quantile(all_preds, 0.70)
        self.sell_thresh = np.quantile(all_preds, 0.30)
        print(f"训练完成，自动阈值 -> 买入: {self.buy_thresh:.5f}, 卖出: {self.sell_thresh:.5f}")
...
def main():
    ...
    ml_strat = MLStrategy(sym, '2020-01-01', '2026-03-20',
                          params=params, seq_len=seq_len,
                          atr_stop=atr_stop, atr_profit=atr_profit)
    strategy.add_execution(ml_strat.exec_fn, [sym])
    ...
    if result.trades is not None and not result.trades.empty:
        result.trades.to_csv('ml_trades_auto_threshold.csv', index=False)
        print(f"交易记录已保存，共 {len(result.trades)} 笔")
    if result.portfolio is not None and not result.portfolio.empty:
        result.portfolio.to_csv('ml_portfolio_auto_threshold.csv')


# ============== 主程序 ==============
if __name__ == '__main__':
    for i in range(10):  # 跑3次，可自行调整
        print(f"\n{'='*20} 第 {i+1} 次回测 {'='*20}")
        main()