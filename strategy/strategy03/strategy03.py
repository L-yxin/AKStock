...
# ---------------------- 特征名称（新增 macd8 和 RSI 超卖） ----------------------
FEATURE_COLS = [
    'ma_5_dist', 'ma_10_dist', 'ma_20_dist', 'ma_60_dist',
    'volatility_10', 'atr_14',
    'volume_ratio', 'volume_trend',
    'macd', 'macd_signal', 'macd_hist', 'macd_diff',  # 原有 （12,26,9）
    'macd8', 'macd8_signal', 'macd8_hist', 'macd8_diff',  # 新增 （8,16,6）
    'rsi_6', 'rsi_14', 'rsi_24',  # 新增 rsi_24
    'rsi_oversold_6', 'rsi_oversold_12', 'rsi_oversold_24',  # RSI 超卖标志
    'k', 'd', 'j',
...
class LSTMTrainer:
    def __init__(self, input_dim):
        self.model = LSTMModel(input_dim).to(device)
        self.buy_thresh = 0.005
        self.sell_thresh = -0.005

    def train(self, df_features, df_target):
        df = df_features.copy()
        df['target'] = df_target
        df = df.dropna()
        ...
        with torch.no_grad():
            ...
            print(f"LSTM训练完成，买入阈值: {self.buy_thresh:.5f}, 卖出阈值: {self.sell_thresh:.5f}")
...
class MLStrategy:
    ...
    def _train_if_needed(self):
        ...
        self.model = LSTMTrainer(input_dim)
        self.model.train(features_df, target)
        self.trained = True
    ...
    def exec_fn(self, ctx: pybroker.context.ExecContext):
        ...
        # 实时计算特征（顺序必须与 FEATURE_COLS 完全一致）
        feature_vals = []
        # 均线距离
        for p in [5, 10, 20, 60]:
        ...
        feature_vals.append(ret.rolling(10).std().iloc[-1])
        ...
        if pos is not None:
            ...
            if not np.isnan(atr_rel[-1]):
                ...
                # 止损
                if float(close[-1]) < avg_cost - 2 * atr_value:
                    ctx.sell_shares = pos.shares
                    self.max_price = None
                    return
                # 移动止盈
                if float(close[-1]) < self.max_price - 1.5 * atr_value:
                    ctx.sell_shares = pos.shares
                    self.max_price = None
                    return
